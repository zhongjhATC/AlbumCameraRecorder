...
class ImageClipWindow : ImageClip() {
     ...
     * 重置过程中不会绘制裁剪框
     */
    var isResetting: Boolean = true

    /**
     * 标记是否显示裁剪框外的遮罩
     * 用于突出显示裁剪区域
     */
    var isShowShade: Boolean = false
    ...
    fun setClipWinSize(width: Float, height: Float) {
        ...
        if (!frame.isEmpty) {
            ImageUtils.center(winFrame, frame)
    ...
    private fun reset(clipWidth: Float, clipHeight: Float) {
        ...
        ImageUtils.fitCenter(winFrame, frame, CLIP_MARGIN)
        // 设置目标裁剪框为当前裁剪框
        targetFrame.set(frame)
    }
    ...
    fun homing(): Boolean {
        ...
        targetFrame.set(frame)
        // 计算裁剪框应该归位到的目标位置
        ImageUtils.fitCenter(winFrame, targetFrame, CLIP_MARGIN)
    ...
    fun getOffsetTargetFrame(offsetX: Float, offsetY: Float): RectF {
        // 创建一个新的矩形对象并设置为目标裁剪框
        val targetFrame = RectF(targetFrame)
        // 应用偏移量
    ...
    fun onDrawShade(canvas: Canvas) {
        ...
        mShadePath.addRect(frame.left + 100, frame.top + 100, frame.right - 100, frame.bottom - 100, Path.Direction.CW)
    ...
    fun getAnchor(x: Float, y: Float): Anchor? {
        ...
        if (Anchor.isCohesionContains(frame, -CLIP_CORNER_SIZE, x, y)
            && !Anchor.isCohesionContains(frame, CLIP_CORNER_SIZE, x, y)
        ) {
            // 初始化锚点值（使用位掩码表示选中的边界）
            var v = 0
            // 获取裁剪框的凝聚力点（四个边界坐标）
            val cohesion = Anchor.cohesion(frame, 0f)
            // 存储触摸点坐标
            ...
            for (i in cohesion.indices) {
                ...
                if (abs((cohesion[i] - pos[i shr 1]).toDouble()) < CLIP_CORNER_SIZE) {
                    v = v or (1 shl i) // 通过位或操作标记选中的边界
                }
            }
    ...
    companion object {
         ...
         * 半透明白色，便于用户对齐图像
         */
        private const val COLOR_CELL = -0x7f000001 // 半透明白色

        /**
         * 裁剪框边框的颜色
         * 纯白色，使裁剪框清晰可见
         */
        private const val COLOR_FRAME = Color.WHITE // 纯白色

        /**
         * 裁剪框角点标记的颜色
         * 纯白色，使角点标记清晰可见
         */
        private const val COLOR_CORNER = Color.WHITE // 纯白色

        /**
         * 裁剪框外遮罩的颜色
         * 半透明黑色，用于突出显示裁剪区域
         */
        private const val COLOR_SHADE = -0x34000000 // 半透明黑色
    }
}
